 ...
 * @author      Kevin Vaughan kevin@kevinvaughan.com -  modify by ArthurH
 * @copyright   2007 Kevin Vaughan - 2012 Arthur Halet
...
class WSDLInterpreter 
{
    ...
    /**
     * Array of entry for YAML conf generator
     * @var array
     * @access private
     */
    private $yamlEntry = array();
    ...
    public function __construct($wsdl) 
    {
        try {
            ...
            //$xpath = new DOMXPath($this->_dom);
            $this->loadLocation($xpath, $this->_dom);
    ...
    private function loadLocation($xpath, &$dom){
            ...
            foreach ($entries as $entry) {
                ...
                /*if(!preg_match('#^http#i', trim($schemaLocation))){
                    $schemaLocation = dirname($this->_wsdl) . "/". $schemaLocation;
                }*/
                $result = @$xsd->load($schemaLocation, 
                    LIBXML_DTDLOAD|LIBXML_DTDATTR|LIBXML_NOENT|LIBXML_XINCLUDE);

                if ($result) {
                    $this->loadLocation(new DOMXPath($xsd), $xsd, $parentEntry);
    ...
    private function _loadClasses() 
    {
        ...
        foreach ($classes as $class) {
            ...
            foreach ($properties as $property) {
                $validateType = $this->_validateType($property->getAttribute("type"));
                $validateName = $this->_validateNamingConvention($property->getAttribute("name"));
                $property->setAttribute("validatedName", $validateName);
                $property->setAttribute("type", $validateType);
                if(stristr($validateType, 'integer') !== FALSE || stristr($validateType, 'double') !== FALSE || 
                    stristr($validateType, 'string') !== FALSE){
                    $this->yamlEntry[$validateName] = array('type_form' => 'input', 'connector' => '');
                    
                }
                if(stristr($validateType, 'boolean') !== FALSE){
                    $this->yamlEntry[$validateName] = array('type_form' => 'select', 'connector' => array("1"=>"true", "0"=>"false"));
                }
            }
            ksort($this->yamlEntry);
    ...
    private function _generateClassPHP($class) 
    {
        ...
        $enumsProp = array();
        foreach ($properties as $property) {
            if($property->getAttribute("type")=='enum'){
                $enumsProp[] = $property->getAttribute("validatedName");
            }else{
                $return .= "\t/**\n"
                     . "\t * @access public\n"
    ...
    private function _generateServicePHP($service) 
    {
        ...
        $return .= 'public function __doRequest($request, $location, $action, $version, $one_way = 0)
    {
        $response = parent::__doRequest($request, $location, $action, $version, $one_way);
        // parse $response, extract the multipart messages and so on
       
        //this part removes stuff
        if(stristr($response,\'<soap\') !== false){
            $start=strpos($response,\'<soap\');
        }else if(stristr($response,\'<s:\') !== false){
        ...
        $return .= "\t".' */'."\n";
        $return .= "\t".'public function __construct($options=array()) {'."\n";
        $return .= "\t\t" .'$wsdl="'.$this->_wsdl.'";'."\n";
    ...
    public function savePHP($outputDirectory) 
    {
        ...
        foreach ($this->_servicePHPSources as $serviceName => $serviceCode) {
            ...
            if (file_put_contents($filename, 
                    "<?php\n\n".$classSource."\n\n".$serviceCode."\n\n?>")) {
                $outputServices[] = $serviceName;
            }
            if(is_file($outputDirectory."/".$serviceName.".yml")){
                $oldYaml = sfYaml::load($outputDirectory."/".$serviceName.".yml");
                if(!empty($oldYaml)){
                    foreach ($oldYaml as $key => $value) {
                        $this->yamlEntry[$key]=$value;
                    }
                }
            }
            if (file_put_contents($outputDirectory."/".$serviceName.".yml", 
                    sfYaml::dump($this->yamlEntry))) {
            }
        }
        $servicesYml = null;
        foreach ($outputServices as $key => $value) {
            $servicesYml[$value] = array('uri'=>$this->_wsdl, 'login'=>null);
        }
        $arrayOutput = array();
        if(is_file((__DIR__)."/../../conf/conf.yml")){
            $oldYaml = sfYaml::load((__DIR__)."/../../conf/conf.yml");
            if(!empty($oldYaml)){
                foreach ($oldYaml as $key => $value) {
                    if(array_key_exists($key, $servicesYml)){
                        $this->yamlEntry[$key]['login']=$value['login'];
                    }
                    
                }
            }
            $arrayOutput['config'] = $oldYaml['config'];
        }
        $arrayOutput['services'] = $servicesYml;
        file_put_contents((__DIR__)."/../../conf/conf.yml", sfYaml::dump($arrayOutput));

        if (sizeof($outputServices) == 0) {
