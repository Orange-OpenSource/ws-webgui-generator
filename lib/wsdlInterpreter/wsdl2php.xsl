...
<xsl:template name="messagePart">
    ...
    <xsl:choose>
        <xsl:when test="string-length($value)!=0">
            <xsl:choose>
                <xsl:when test="$refnode[@maxOccurs='unbounded']">
                    <entry debug="2" name="{$value}" type="enum" />
                </xsl:when>
                <xsl:otherwise>
                    <entry debug="2" name="{$value}" type="enum" />
                </xsl:otherwise>
        ...
        <xsl:otherwise>
            <xsl:choose>
                <xsl:when test="$refnode[@maxOccurs='unbounded']">
                    <entry debug="6" name="{$name}" type="{substring-after($type,':')}" error="uninterpreted" fname="{name($refnode)}" lname="{local-name($refnode)}" uri="{namespace-uri($refnode)}" />
                </xsl:when>
                <xsl:otherwise>
                    <entry debug="6" name="{$name}" type="{substring-after($type,':')}" error="uninterpreted" fname="{name($refnode)}" lname="{local-name($refnode)}" uri="{namespace-uri($refnode)}" />
                </xsl:otherwise>
